...
THREE.PointDragControls = function(){
    ...
    this.globals = {                                    // globals - i.e. scoped across whole of PointControls
                                                        // They *can* be accessed externally via THREE.PointControls.globals
    ...
    this.init = function(scene,camera,renderer,options){
        ...
        renderer.domElement.addEventListener('mousedown', function(e) {
            ...
            g.pointer = get_pointer_v({x:e.clientX,y:e.clientY});
            g.last_pointer_posn = { x: e.offsetX, y: e.offsetY };                
            
            var button = detect_mouse_button(e);
            if ( typeof button != 'undefined' ){
                var object_clicked = set_raycast_intercept(g.pointer,camera);
                if ( object_clicked ){
                    if( button == 'right'){
                        g.active_axes = {
                            r: 'z',
        ...
        renderer.domElement.addEventListener('touchstart', function(e){
            ...
            var start_posn = {x:e.changedTouches[0].clientX,y:e.changedTouches[0].clientY};
            g.pointer = get_pointer_v( start_posn );
            var object_clicked = set_raycast_intercept(g.pointer,camera);
            if (object_clicked){
                g.origin_touch_id = e.changedTouches[0].identifier;
                g.last_pointer_posn = start_posn;            
        ...
        renderer.domElement.addEventListener('touchmove', function(e){
            ...
            if ( typeof g.intersect.forward != 'undefined' && typeof g.last_pointer_posn != 'undefined'){
                ...
                if ( typeof touch_num != 'undefined' ){        
                    var new_pointer_posn={ 
                        x:    e.changedTouches[touch_num].clientX,                    
                        y:    e.changedTouches[touch_num].clientY
                    };

                    g.dt = {
                        x: new_pointer_posn.x-g.last_pointer_posn.x,
                        y: new_pointer_posn.y-g.last_pointer_posn.y
                    };
                    if (e.touches.length == 1 && axes_locked() ){
                        check_active_axes();
                    }
                    if ( typeof g.active_axes.r != 'undefined' ){
                        transform_object(new_pointer_posn);
                    }
                    g.last_pointer_posn = new_pointer_posn;
                }
            }     
        });
        ...
        renderer.domElement.addEventListener('mousemove',function(e){
            ...
            if ( typeof g.intersect.forward != 'undefined'){
                var new_pointer_posn = {x:e.offsetX,y:e.offsetY};
                g.dt = {
                    x: new_pointer_posn.x-g.last_pointer_posn.x,
                    y: new_pointer_posn.y-g.last_pointer_posn.y
                };
        ...
        function transform_object(){ //either the rotation or translation, depending on mode
            
            var transform;
            if ( g.mode == 'translate' ){

                transform = get_translation_transform();

            } else if ( g.mode == 'rotate' ){

                transform = get_rotation_transform();
        ...
        function get_translation_transform(){
            ...
            var origin_rel = translation_origin.clone().sub(camera.position.clone());
            var origin_rel_z = origin_rel.dot( camera.getWorldDirection() );

            if ( g.active_axes.t.match(/x/) ){ tamt.x = get_translation_distance(origin_rel_z,h,g.dt.x); }
            if ( g.active_axes.t.match(/y/) ){ tamt.y = - get_translation_distance(origin_rel_z,h,g.dt.y); }
            if ( g.active_axes.t.match(/z/) ){ tamt.z = g.dt[p.z_control_axis] / p.z_shift_distance; }
                
            return {
                origin: translation_origin,
                matrix: new THREE.Matrix4().makeTranslation(tamt.x,tamt.y,tamt.z)
            };
        }
        ...
        function get_rotation_transform(){
           var rotation_origin = new THREE.Vector3(
            ...
            var origin_v = rotation_origin.clone().sub(camera.position.clone());
            var up4 = new THREE.Vector4(camera.up.x,camera.up.y,camera.up.z,0
            ...
            var rel = {
                x: origin_v.dot(cam_horiz_v),
                y: origin_v.dot(up),
                z: origin_v.dot( camera.getWorldDirection() )
            };            
